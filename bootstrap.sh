...
if grep -q s3fs /etc/fstab; then :
        ...
        echo "s3fs#general-cr3tu /home/ubuntu/data fuse _netdev,allow_other,iam_role=auto,uid=1000,gid=1000  0  0" >> /etc/fstab
        mount -a
fi
...
su - ubuntu -c "npm install -g npm@latest pm2"
